...
- **修正**單精度值的字串形式：由 `float` 宣告 tag 送出、再由 `string` 宣告 tag 讀取的值，其字串取自 32 位元值（`"25.34"`），而非加寬後的 double（`"25.34000015258789"`）。兩種字串解析回同一個單精度值，但僅前者符合 `float` tag 實際的精度。此問題自 2.0.0 起存在，且會在任何將該 tag 宣告為 string 的節點上顯現，無論位於下游何處。
