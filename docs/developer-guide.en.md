...
- **Fixed** the string form of a single-precision value: a value carried by a `float`-declared tag and read through a `string`-declared tag is rendered from its 32-bit value (`"25.34"`) rather than from the double it was widened to (`"25.34000015258789"`). Both texts parse back to the same single-precision value; only the first matches the precision a `float` tag actually has. Present since 2.0.0, and visible at any node that declares the tag a string, however far downstream.
