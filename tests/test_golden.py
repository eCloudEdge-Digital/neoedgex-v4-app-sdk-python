...
* ``envelope`` pins the message layout byte for byte in both directions.
"""
...
GOLDEN_PATH = Path(__file__).parent / "testdata" / "golden" / "neoflow_v2_1_0.json"
...
def test_envelope_encode_is_byte_exact() -> None:
...
def test_envelope_decode_returns_source_timestamp_and_data_span() -> None:
    envelope = GOLDEN["envelope"]
...
def test_fixture_declares_its_go_source() -> None:
    # Python 2.0.0 claims equivalence with Go v2.1.0; if the fixture is ever
    # regenerated from another tag, that claim (and the whole file) moves.
    assert "v2.1.0" in GOLDEN["go_sdk"]
    assert GOLDEN["regen"]
